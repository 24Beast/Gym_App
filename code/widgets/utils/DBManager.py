...
from widgets.utils.tools import (
    DateToDateTime,
    memberDataToListItem,
    pendingDataToTableItems,
)
...
class DBManager:
    ...
    def searchInfo(self, string: str) -> None:
        ...
        self.infoDocs = self.infoCollection.find(
            {
                "$or": [
                    {"MemId": {"$regex": str(memNum), "$options": "i"}},
    ...
    def getMemNum(self, name: str) -> int:
        ...
        memNum = number
        for doc in self.infoCollection.find(
            {"MemId": {"$gt": (number - 1), "$lt": (number + self.MULTIPLIER)}}
        ).sort("MemId", -1):
            memNum = doc["MemId"] + 1
            print(memNum)
            break
    ...
    def addCalendar(self, date, MemId) -> None:
        ...
        self.calendarCollection.update_one(
            {"Date": date}, {"$addToSet": {"MemIds": MemNum}}
        )
    ...
    def fetchCalender(self, MemId) -> list:
        MemNum = self.memIdToMemNum(MemId)
        dates = []
        for doc in self.calendarCollection.find(
            {"MemIds": {"$elemMatch": {"$eq": MemNum}}}
        ):
